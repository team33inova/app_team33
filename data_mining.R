...
db_inova <-  dbConnect(RSQLite::SQLite(), 'inova.sdb')
db_inova_dm <- dbConnect(RSQLite::SQLite(), 'inova_dm.sdb')


#baixar tabelas dimensao no R
#sgt classes
sgt_classe <- read.csv(file = 'dim/sgt_classe.csv', sep = ';', header = T, encoding = 'UTF-8')
sgt_assunto <- read.csv(file = 'dim/sgt_assunto.csv', sep = ';', header = T, encoding = 'UTF-8')
sgt_mov <- read.csv(file = 'dim/sgt_mov.csv', sep = ';', header = T, encoding = 'UTF-8')
serventias <- read.csv(file = 'dim/serventias.csv', sep = ';', header = T, encoding = 'UTF-8')
anomes <- read.csv(file = 'dim/anomes.csv', sep = ';', header = T, encoding = 'UTF-8')
tipo_assunto <- read.csv(file = 'dim/tipo_assunto.csv', sep = ';', header = T, encoding = 'UTF-8')


#Testar tabela e gravar tabelas dimensao na base DATAJUD. apagar objeto R
dbExecute(db_inova, 'DROP TABLE IF EXISTS sgt_classe')
dbWriteTable(db_inova,"sgt_classe",sgt_classe)
rm(sgt_classe)

dbExecute(db_inova, 'DROP TABLE IF EXISTS sgt_assunto')
dbWriteTable(db_inova,"sgt_assunto",sgt_assunto)
rm(sgt_assunto)

dbExecute(db_inova, 'DROP TABLE IF EXISTS sgt_mov')
dbWriteTable(db_inova,"sgt_mov",sgt_mov)
rm(sgt_mov)

dbExecute(db_inova, 'DROP TABLE IF EXISTS serventias')
dbWriteTable(db_inova,"serventias",serventias)
rm(serventias)
...
on IFNULL(mov.codigoNacional, mov.codigoPaiNacional) = sgt.cod_mov
where  substr(dataHora,1,4) BETWEEN '1980' and '2021'
                      ")
...
tmp_df <-  dbGetQuery(db_inova, 
                      ...
                      on ifnull(ass.codigoNacional, ass.codigoPaiNacional) = sgt.cod_assunto;
                      ")
